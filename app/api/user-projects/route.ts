...
export async function POST(req: NextRequest) {
  try {
    const { user_id, name, description, company_name, representative_name, region } = await req.json();
    if (!user_id || !name?.trim()) return NextResponse.json({ error: '사용자 ID와 항목 이름은 필수입니다.' }, { status: 400 });

    const supabase = getServiceClient();
    const { data, error } = await supabase
      .from('user_projects')
      .insert({ user_id, name: name.trim(), description, company_name: company_name?.trim() || null, representative_name: representative_name?.trim() || null, region: region?.trim() || null })
      .select('*')
      .single();
...
export async function PATCH(req: NextRequest) {
  try {
    const { id, name, description, company_name, representative_name, region } = await req.json();
    if (!id || !name?.trim()) return NextResponse.json({ error: 'id와 이름은 필수입니다.' }, { status: 400 });

    const supabase = getServiceClient();
    const { data, error } = await supabase
      .from('user_projects')
      .update({ name: name.trim(), description: description || null, company_name: company_name?.trim() || null, representative_name: representative_name?.trim() || null, region: region?.trim() || null })
      .eq('id', id)
      .select('*')
      .single();
