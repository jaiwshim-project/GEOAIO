...
export async function POST(request: NextRequest) {
  try {
    ...
    const {
      content, title, tone, topic, continuation, previousContent,
    ...
    // ⭐ Phase 2: Spoke 글일 때 series 강제 블록 (angle 유지·카탈로그 회귀 방지)
    const seriesBlock = (seriesRole === 'spoke' && seriesAngle) ? `

⭐⭐⭐ 시리즈 시점 (가장 우선) ⭐⭐⭐
이 글은 시리즈의 SPOKE 편으로, 각도 "${seriesAngle}" (intent: ${seriesIntent || 'unspecified'})만을 다루는 글입니다.
이어 쓰는 부분도 반드시 이 angle 관점에서만 작성하세요.

[Spoke 강제 규칙]
- 추가하는 H2·소제목은 모두 angle "${seriesAngle}"의 단면을 다루는 제목이어야 함.
- ❌ "GEO와 SEO 차이", "5대 원리", "10대 KPI", "5가지 약점" 같은 카탈로그·일반 입문형 제목 금지.
- ✅ angle을 잘게 쪼갠 H2 (예: angle "30분 단계별 실행" → [준비물 / 단계 1 / 단계 2 / 검증 / 흔한 함정])
- FAQ Q를 추가한다면 angle 맥락의 구체 질문만 (Pillar의 일반 입문 Q 금지).
- 비교표를 추가한다면 angle 관점 비교만.
${seriesPillarCatalog && Array.isArray(seriesPillarCatalog) && seriesPillarCatalog.length > 0 ? `
[1편 Pillar에서 이미 다룬 항목 — 본문 H2·소제목으로 재사용 금지]
${seriesPillarCatalog.map((c: string) => `  · "${c}"`).join('\n')}
` : ''}
