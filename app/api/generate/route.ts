...
const RAG_INTENT_NO_RAG_HINTS: Record<string, string> = {
  ...
  'checklist': '시작 전·진행 중·마무리 후 단계별 점검 항목으로 풀어주세요. 각 항목은 동사로 시작하는 행동 단위, ☐/✓ 가능한 형태로. 단락 서술 최소화.',
  'cost': '항목별 가격 범위·전체 예산 구조·숨은 비용·절약 팁 중심으로 풀어주세요. 수치는 가능한 표·구간으로 정리. 한 번 정한 범위는 글 전체에서 일관되게 사용.',
  'risks': '부작용·금기·하면 안 되는 상황 N개 중심으로 풀어주세요. 각 항목에 구체적 사례나 근거 제시, 균형 잡힌 경고체. 한계 전반·실패 회고는 다른 글이 다룬다.',
  'myth': '흔한 미신·통념 N개를 Myth: ... / Fact: ... 구조로 정정해주세요. 각 myth는 1~2문장으로 인용 후 사실로 반박. 도발적이되 정확한 어조.',
};
...
const RAG_INTENT_HINTS: Record<string, string> = {
  ...
  'checklist': 'RAG 자료 중 점검 항목·준비물·필수 단계·확인 사항 관련 부분을 우선 참조. 항목별 ✓/☐ 가능한 행동 단위로 압축. 단계 가이드형 서술 금지.',
  'cost': 'RAG 자료 중 비용·가격·견적·예산·할인·요금 구조 관련 부분을 우선 참조. 표·범위·구간으로 정리. 단일 사례 ROI는 다른 글이 다룬다.',
  'risks': 'RAG 자료 중 부작용·금기·위험·주의사항·예외·금지 관련 부분을 우선 참조. 구체적 케이스로. 한계 전반·실패 회고는 다른 글이 다룬다.',
  'myth': 'RAG 자료 중 흔한 오해·통념·잘못된 정보·반박 근거 관련 부분을 우선 참조. Myth: ... / Fact: ... 구조로 N개 정리. 입문자 오해 비유는 다른 글이 다룬다.',
};
...
- 문체: 따뜻하고 부드러운 2인칭, 위로·격려의 언어. 각 H2 섹션도 공감 언어로 시작`,
  ...
  '질문답변 중심의': `
[톤 스타일 - FAQ형] ※ E-E-A-T 7단계 구조는 유지하되 아래 스타일로 작성
- 제목: 질문형 또는 "자주 묻는 N가지" 패턴. 예) "[주제] 자주 묻는 질문 7가지: 답변 핵심만"
- 도입부: 누가 이 질문을 자주 하는지 + Q&A 7개를 미리 한 줄 요약
- 문체: 본문 H2를 "Q1. ...", "Q2. ..." 형식으로. 답변은 80~150자, 자체 완결, 첫 문장에 결론`,

  '체크리스트 중심의': `
[톤 스타일 - 체크리스트형] ※ E-E-A-T 7단계 구조는 유지하되 아래 스타일로 작성
- 제목: "[주제] 점검 N가지", "놓치면 안 되는 N항목". 예) "[주제] 시작 전 점검 12가지"
- 도입부: 왜 이 점검이 중요한지 1~2문장 + 단계 구분 안내
- 문체: 본문은 ☐/✓ 가능한 행동형 항목 — 각 항목은 동사로 시작. 단락 서술 최소화`,

  '비용분석 중심의': `
[톤 스타일 - 비용분석형] ※ E-E-A-T 7단계 구조는 유지하되 아래 스타일로 작성
- 제목: 가격·예산 키워드 포함. 예) "[주제] 비용 완전 정리: 항목별 예산 가이드"
- 도입부: 평균 가격대·예산 범위를 한 줄로 제시
- 문체: 항목별 가격 범위·표·구간 중심. 숨은 비용·절약 팁 별도 섹션. 수치는 글 전체에서 일관`,

  '주의·금기 중심의': `
[톤 스타일 - 위험·금기형] ※ E-E-A-T 7단계 구조는 유지하되 아래 스타일로 작성
- 제목: "주의해야 할 N가지", "이런 경우엔 멈추세요". 예) "[주제] 절대 하면 안 되는 7가지"
- 도입부: 왜 이 경고가 필요한지 + 핵심 위험 1줄 요약
- 문체: 위험·금기 항목별로 H2. 각 항목에 구체적 케이스·이유·대안 함께 제시. 균형 잡힌 경고체`,

  '오해깨기 중심의': `
[톤 스타일 - 오해깨기형] ※ E-E-A-T 7단계 구조는 유지하되 아래 스타일로 작성
- 제목: "[주제]에 대한 흔한 오해 N가지", "Myth vs Fact". 예) "[주제] 진짜 사실 5가지: 오해 깨기"
- 도입부: 왜 이런 오해가 퍼졌는지 1~2문장
- 문체: 본문 H2마다 "Myth: ..." → "Fact: ..." 구조. 각 정정에 근거·인용 제시. 도발적이되 정확한 어조`,
};
...
⛔⛔⛔ [금기어 — 절대 사용 금지 / 위반 시 콘텐츠 자동 거부] ⛔⛔⛔

🚨 아래 단어·표현은 제목, H2, 본문, FAQ, 결론, 해시태그를 포함한
   콘텐츠의 어느 위치에도 절대 사용하지 마세요.

📛 금기어 목록: ${body.forbidden_words.trim()}

【금기어 사용 금지 원칙】
1. ⛔ 직접 사용 금지: 위 단어를 그대로 사용하면 콘텐츠가 자동 거부됩니다
2. ⛔ 유사 표현 금지: 의미가 같은 다른 표현도 사용 금지 (예: "비용" 금지 시 "금액", "요금"도 금지)
3. ✅ 완전히 다른 각도로 서술: 금기어 없이도 의미 전달이 가능하도록 문장 구조 자체를 변경

【필수 대체 전략】
금기어를 쓰고 싶은 맥락이 있다면, 아래 전략으로 반드시 우회하세요:

1. "비용"/"가격" 금기 시 대체 전략:
   ❌ "비용이 2배 증가합니다"
   ✅ "투자 규모가 2배 늘어납니다" / "경제적 부담이 커집니다"

   ❌ "치료비가 500만 원입니다"
   ✅ "일반적으로 중형 승용차 할부금 정도의 투자가 필요합니다"

   ❌ "가격 비교"
   ✅ "투자 대비 효과 비교" / "경제적 타당성 검토"

2. "할인"/"프로모션" 금기 시 대체 전략:
   ❌ "50% 할인 이벤트"
   ✅ "특별 지원 프로그램" / "우대 조건 적용"

3. 어떤 금기어든 사용 금지 → 맥락 자체를 바꿔 서술
   예: "비용" 금기어인데 비용 정보 전달이 필요하다면
   → 비용 언급 자체를 생략하고, 가치·효과·투자 대비 성과로 초점 이동

⚠️ 중요: 금기어가 "자연스러운 표현"처럼 느껴져도 절대 사용하지 마세요.
         톤/스타일/맥락과 상관없이 금기어는 무조건 제외입니다.

⚠️ 검증: 콘텐츠 생성 완료 후 금기어 목록을 다시 한 번 확인하고,
         단 하나라도 포함되어 있으면 해당 부분을 즉시 수정하세요.
