...
export async function POST(request: NextRequest) {
  try {
    ...
    // Claude Haiku 4.5 + prompt caching 단독 사용 (md 초안 생성).
    // EEAT 7단계 변환도 동일 모델 — /api/convert-eeat
    const claudeKey = getClaudeKey(request);
    const geminiKey = getGeminiKey(request);
    void geminiKey; // geminiKey는 이 라우트에서 사용하지 않음 (api-auth 공유 import 안전성 유지)
...
` : '';
    ...
    // ── 정적 prefix(10톤 공통, 캐싱 대상) + 톤 suffix(콜마다 변동, 캐싱 안함) 분리 ──
    let staticUserPrefix: string;

    if (hasRag) {
      // RAG 기반 생성
      const ragContent = pFiles!.map(f => `▶ ${f.file_name}\n${f.content}`).join('\n\n');
      staticUserPrefix = `${cepBlock}${caseBlock}${contactCtaBlock}[프로젝트 RAG 지식 기반]
...
- 제목: 아래 명시된 톤 + 수치 포함 (예: "95% 달성", "3배 향상")
- RAG 자료의 사실·수치·표현을 반드시 본문에 녹여 작성
- 아래 명시된 톤을 제목부터 마지막까지 일관 유지
...
다음 JSON 형식 그대로 출력하세요. 코드블록(\`\`\`)·설명문·인사말 일체 금지. 첫 글자는 반드시 '{'.
{
  ...
  "content": "마크다운 본문 전체. 1700자 이상 2300자 이내. H1 제목은 별도 title 필드에 있으므로 본문은 ## H2부터 시작.",
  ...
  "metadata": {
    ...
    "seoTips": ["팁1", "팁2", "팁3"]
      ...
      // 일반 생성 (RAG 없음)
      staticUserPrefix = `${cepBlock}${caseBlock}${contactCtaBlock}다음 조건에 맞는 ${categoryLabel} 콘텐츠를 생성해주세요.
...
${body.additionalNotes ? `\n추가 요청사항:\n${body.additionalNotes}\n` : ''}
[필수 사항]
- 제목은 아래 명시된 톤 가이드에 맞게 작성하세요.
- 본문 전체의 문체·구조·어조가 아래 명시된 톤을 일관되게 반영해야 합니다.
...
다음 JSON 형식 그대로 출력하세요. 코드블록(\`\`\`)·설명문·인사말 일체 금지. 첫 글자는 반드시 '{'.
{
  ...
  "metadata": {
    ...
    // 톤 suffix — 콜마다 다름 (캐싱 안 함)
    const toneSuffix = `

[적용할 톤/스타일]
${toneDesc}${toneGuide}`;

    // Claude Haiku 4.5 + prompt caching (md 초안 생성 전용).
    let text = '';
    try {
      if (!claudeKey) {
      ...
      console.log('[API] Claude Haiku 4.5 콘텐츠 생성');
        ...
        messages: [
          {
            ...
            content: [
              {
                // 10톤 공통 정적 prefix — RAG·CEP·회사정보·구조 spec·JSON 형식 모두 포함
                type: 'text' as const,
                text: staticUserPrefix,
                cache_control: { type: 'ephemeral' as const },
              },
              {
                // 톤별 suffix — 캐싱 안 함
          ...
          { role: 'assistant', content: '{' }, // prefill — JSON 시작 강제
        ],
      });
      const raw = message.content[0]?.type === 'text' ? message.content[0].text : '';
      // prefill로 시작한 '{' 가 응답에 포함되지 않으므로 prepend
      text = '{' + raw;
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const usage = (message as any).usage;
      if (usage) {
        console.log('[API] Claude usage:', {
      ...
      console.error('[API] Claude 실패:', msg);
      return withCors(NextResponse.json(
