...
async function runTest() {
  console.log('🚀 콘텐츠 생성 테스트 시작 (개선된 버전)\n');
  console.log('테스트 정보:');
  console.log(`- 주제: ${testData.topic}`);
  console.log(`- 카테고리: ${testData.category}`);
  console.log(`- 톤: 10가지`);
  console.log(`- 반복: 3회`);
  console.log(`- 요청 간격: 10초 (Rate Limit 회피)\n`);
  ...
  for (let run = 1; run <= 3; run++) {
    ...
    for (let i = 0; i < tones.length; i++) {
      ...
      try {
        ...
        console.log(`${status} [${i + 1}/10] ${result.tone.padEnd(12)} - Status: ${result.status} ${content}`);

        // 마지막 요청이 아니면 딜레이 (Rate Limit 회피)
        if (i < tones.length - 1) {
  ...
  results.forEach(r => {
    const bar = '█'.repeat(Math.floor(r.successRate / 10)) + '░'.repeat(10 - Math.floor(r.successRate / 10));
    console.log(`시도 #${r.run}: ${r.successCount}/${r.totalCount} (${r.successRate}%) [${bar}]`);
  });
